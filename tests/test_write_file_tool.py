...
from nano_code.agent_tool.os_tool.write_file import WriteFileTool
...
def test_write_file_outside_root(tmp_path):
    ...
    tool = WriteFileTool.init()
    result = asyncio.run(tool._execute(session, args))
    assert "not within the working directory" in result.for_llm or "not within the working directory" in result.for_human
